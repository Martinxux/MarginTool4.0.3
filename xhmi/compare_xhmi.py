...
def compare_data_xhmi(files, file_path, file_list, log_func=None):
    data_list = []
    result_list = []
    for file in files:
    ...
    with open(file_path, 'a+') as file:
        ...
        # 添加 Worst Case (all lanes) 标签并写入文本文件
        min_values_all_lanes = {}
        for result in result_list:
            for i, value in enumerate(result):
                key = f"数值 {i + 1}"
                if key not in min_values_all_lanes or value < min_values_all_lanes[key]:
                    min_values_all_lanes[key] = value

        result_dict_all_lanes = {'Worst Case (all lanes)': min_values_all_lanes}
        file.write('Worst Case (all lanes),' + ','.join(
            [str(r) for r in result_dict_all_lanes['Worst Case (all lanes)'].values()]) + '\n')
